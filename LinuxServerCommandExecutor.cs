public class LinuxServerCommandExecutor : IServerCommandExecutor
{
	...
	public LinuxServerCommandExecutor(ITestRunner runner)
	{
		_runner = runner;
	}
		...
		using (var client = new SshClient(host, username, password))
		{
			try
			{
				...
				using (var stream = client.CreateShellStream(TerminalName, Columns, Rows, Width, Height, BufferSize, modes))
				{
					...
					if(appId != null)
					{
						stream.Write($"dzdo -k su - {appId}\n");
						stream.Expect($"[dzdo] password for {username}:");
